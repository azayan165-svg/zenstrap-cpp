...
namespace odessa
{
    ...
    class c_memory
    {
      ...
      public:
        ...
        /**
         * @brief Checks if the process is attached and valid
         *
        ...
        /**
         * @brief Checks if an address is valid for reading
         *
         * @param address The address to check
         * @return true if valid, false otherwise
         */
        [[nodiscard]] bool is_valid( const std::uint64_t address ) const noexcept
        {
            if ( !m_process || !address )
                return false;

            MEMORY_BASIC_INFORMATION mbi = { };
            if ( VirtualQueryEx( m_process, reinterpret_cast< LPCVOID >( address ), &mbi, sizeof( mbi ) ) == 0 )
                return false;
        ...
        /**
         * @brief Safely reads memory with validation (template version)
         *
         * @tparam type_t The type of data to read
         * @param address The address to read from
         * @param out Reference to store the read value
         * @return true if read successful, false otherwise
         */
        template < typename type_t >
        [[nodiscard]] bool read_safe( const std::uint64_t address, type_t &out ) const noexcept
        {
            if ( !is_valid( address ) )
                return false;

            SIZE_T bytes_read = 0;
            BOOL   result     = ReadProcessMemory( m_process, reinterpret_cast< LPCVOID >( address ), &out, sizeof( type_t ), &bytes_read );

            return result != 0 && bytes_read == sizeof( type_t );
        }

        /**
         * @brief Safely reads a buffer with validation
         *
         * @param address The address to read from
         * @param size The number of bytes to read
         * @return Vector of bytes read, empty on failure
         */
        [[nodiscard]] std::vector< std::uint8_t > read_safe( const std::uint64_t address, std::size_t size ) const noexcept
        {
            if ( !is_valid( address ) || size == 0 || size > 1024 * 1024 ) // 1MB limit
                return { };

            std::vector< std::uint8_t > buffer( size );
            SIZE_T                      bytes_read = 0;

            BOOL result = ReadProcessMemory( m_process, reinterpret_cast< LPCVOID >( address ), buffer.data( ), size, &bytes_read );
